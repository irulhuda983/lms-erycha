...
<template>
    <div class="w-full box-border p-5 mt-14">
        <div class="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
            <div
                class="box-border px-6 py-6 border-b bg-white flex items-center"
            >
                <!-- <h2 class="text-lg font-bold">Tes</h2> -->
                <button
                    class="block rounded text-xs px-2 py-1.5 font-semibold text-green-600 border border-green-200 bg-green-200 mr-4"
                >
                    Aktif Tes
                </button>
                <button
                    @click.prevent="router.push({ name: 'siswaHasilUjian' })"
                    class="block rounded text-xs px-2 py-1.5 font-semibold text-gray-600 border border-gray-200 bg-gray-200 hover:text-green-600 hover:border-green-200 hover:bg-green-200"
                >
                    Hasil Tes
                </button>
                    ...
                    >
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">
                                <span class="bg-gray-300 text-gray-300 rounded"
                                    >Nama Tes</span
                                >
                            </th>
                            <th scope="col" class="px-6 py-3">
                        ...
                        >
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="w-full flex justify-end space-x-2">
                                    <a
                                        href="#"
                                        class="rounded-lg overflow-hidden block font-normal bg-gray-300 text-gray-300"
                                    >
                                        <svg
                ...
                >
                                    ...
                                    >
                                        <button
                                            v-if="
                                                ...
                                                item.is_active == 1
                                            "
                                            @click.prevent="showModal(item.id, item.status)"
                                            class="capitalize block font-normal text-gray-600 rounded text-xs px-2 py-1.5 font-semibold"
                                            :class="item.status == 'mulai_kerjakan' ? 'bg-green-200 text-green-500 hover:text-white hover:bg-green-400':'bg-orange-200 text-orange-500 hover:text-white hover:bg-orange-400'"
                                        >
                                            {{ item.status == 'mulai_kerjakan' ? 'mulai tes' : 'Lanjut tes' }}
                                        </button>
                                        <button
                                            v-else
                                            class="capitalize cursor-not-allowed block font-normal text-gray-600 border border-gray-200 rounded text-xs px-2 py-1.5 bg-red-200 text-red-500 font-semibold"
                                        >
                    ...
                    <tbody v-else>
                        ...
                        >
                            <td
                                colspan="7"
                                class="text-center px-6 py-4 font-normal text-gray-900 whitespace-nowrap dark:text-white italic"
                            >
    ...
    <Modal size="xl" v-if="isShowModal" @close="closeModal">
        <template #header>
            <div class="flex items-center text-lg">Detail Tes</div>
        </template>
        <template #body>
            <div class="mb-3">
                <label
                ...
                >
                <div
                    class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-2.5 py-1.5"
                >
                    {{ detailTes.jenis_ujian }}
                </div>
            </div>
            <div class="mb-3">
                <label
        ...
        <template #footer>
            ...
            <div v-else class="flex justify-end">
                ...
                >
                    {{ statusTes == 'mulai_kerjakan' ? 'Mulai Kerjakan' : 'Lanjut Kerjakan' }}
                </button>
            </div>
        </template>
    </Modal>
