...
const fetchData = async () => {
    ...
    try{
        const { data } = await axios.get('/admin/mapel', { params })
...
const storeData = async () => {
    ...
    try{
        ...
        const { data } = await axios.post(`/admin/mapel`, payload)
        fetchData()
        closeModalAdd()
    }catch(e) {
        if(e.response.status == 401) {
...
const showModalEdit = async (id) => {
    try{
        const { data } = await axios.get(`/admin/mapel/${id}/detail`)
...
const editData = async () => {
    ...
    try{
        ...
        const { data } = await axios.post(`/admin/mapel/${payload.id}/update`, payload)
        fetchData()
        closeModalEdit()
    }catch(e) {
        if(e.response.status == 401) {
...
const deleteData = async () => {
    ...
    try{
        ...
        const { data } = await axios.delete(`/admin/mapel/${payload.id}/delete`, payload)
        fetchData()
        closeModalDelete()
    }catch(e) {
        if(e.response.status == 401) {
...
<template>
    <div class="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
        <div class="box-border px-6 py-3 border-b bg-white flex justify-between items-center">
            <h2 class="text-lg font-bold">Mapel</h2>
            ...
            <table v-if="isLoading" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 border-b">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">
                            <span class="bg-gray-300 text-gray-300 rounded">Kode Mapel</span>
                        </th>
                        <th scope="col" class="px-6 py-3">
                            <span class="bg-gray-300 text-gray-300 rounded">Kode Mapel</span>
                        </th>
                ...
                <tbody>
                    <tr v-for="i in 5" :key="i" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 font-normal text-gray-900 whitespace-nowrap dark:text-white">
                            <span class="bg-gray-300 text-gray-300 rounded">nama Mapel</span>
                        </td>
    ...
    <Modal size="xl" v-if="isShowModalAdd" @close="closeModalAdd">
      ...
      <template #body>
        <div class="mb-6">
            <label for="kode" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kode Mapel</label>
        ...
        <div class="mb-6">
            <label for="nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Mapel</label>
            ...
            <div class="w-full bg-white">
                <div class="w-full flex space-x-5">
                    ...
                    <div class="flex-1">
                        ...
                        Apakah Anda yakin ingin menghapus mapel {{ payload.nama }} ?
                        </div>
