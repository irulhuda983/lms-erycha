            ...
            <div class="w-full box-border rounded-[10px] shadow-2xl bg-white p-4">
                ...
                <div class="w-full box-border flex items-center justify-between border-b border-gray-300 pb-3">
                    ...
                    <div class="w-1/2 box-border flex items-center justify-end">
                        <div class="rounded-full px-4 py-1.5 text-xs border border-red-500 text-gray-600 mr-0 lg:mr-5 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>

                            <div class="mr-2">Siswa Waktu :</div>

                            <div class="font-semibold">{{ hoursDisplay }}:{{ minutesDisplay }}:{{ secondsDisplay }}</div>
                        </div>

                        <button class="rounded-full px-4 py-1.5 text-xs bg-[#7F83FF] text-white flex items-center justify-center" @click.prevent="openModalDaftar()">
                ...
                <div class="w-full box-border flex items-center justify-between box-border py-4 pt-8 px-0 lg:px-10">
                    <button @click.prevent="prevSoal()" class="rounded-full px-8 py-2 flex items-center justify-center font-semibold text-red-600 bg-red-300 hover:bg-red-600 hover:text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-3 h-3 mr-2">
                    ...
                    <button @click.prevent="raguRagu()" class="cursor-pointer rounded-full px-8 py-2 flex items-center justify-center font-semibold text-orange-600 bg-orange-300 hover:bg-orange-600 hover:text-white">
                    ...
                    <button
                        ...
                        class="rounded-full px-8 py-2 flex items-center justify-center font-semibold text-blue-600 bg-blue-300 hover:bg-blue-600 hover:text-white"
                    >
                        <div class="mr-2">Selanjutnya</div>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-3 h-3">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                        </svg>
                    </button>

                    <button v-else @click.prevent="openModalSelesai()" class="rounded-full px-8 py-2 flex items-center justify-center font-semibold text-blue-600 bg-blue-300 hover:bg-blue-600 hover:text-white">
                        <div class="mr-2">Selesai</div>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-3 h-3">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                        </svg>
                    </button>
        ...
        <div class="w-full lg:w-[700px] bg-white rounded overflow-x-hidden shadow-lg">
            ...
            <div class="w-full p-4 box-border">
                <div class="w-full box-border grid grid-cols-5 lg:grid-cols-10 gap-3">
                    <div v-for="i in parseInt(ujianSiswa.soal.jml_pg)" :key="i" @click.prevent="changeNomorSoal(i)" class="cursor-pointer box-border flex items-center justify-center">
                        <div
                            class="w-14 h-14 rounded box-border border border-gray-400 flex items-center justify-center"
                            :class="nomorSoal == i ? 'bg-gray-400 text-white' : 'text-gray-400'"
                        >
                            <span class="font-semibold lg:text-lg">{{ i }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="w-full p-4 box-border">
                <div class="w-full box-border">
                    ...
                    <div class="w-full flex items-center justify-center">
                        <button @click.prevent="stopTes()" class="rounded-full px-8 py-2 flex items-center justify-center font-semibold text-white bg-red-500 hover:bg-red-800 mr-3">Selesai Tes</button>
                        <button @click.prevent="closeModalSelesai()" class="rounded-full px-8 py-2 flex items-center justify-center font-semibold text-white bg-green-500 hover:bg-green-800">Kembali</button>
                    </div>
                </div>
            </div>
        </div>
